...
- **弹幕去重**：自动合并 30 秒窗口内重复或相似的弹幕，合并后以 `[xN]` 标注重复次数，减少刷屏干扰（默认开启，直播时不生效）
- **启动加速**：视频详情请求并行化拆分，历史记录已携带 cid 时预取播放地址；首页首帧渲染后再延迟预热 MPV，缩短进入播放前的等待

### 🌐 播放线路选择
- 在播放设置中为视频与直播选择 CDN 线路
- 支持自动、备用线路 1–3，以及指定 CDN 节点（阿里云 / 腾讯云 / 华为云 / Akamai 海外 等）
- 选定节点时重写媒体 URL 主机并保留原地址作为回退，直播播放同时将全部流地址作为备用，提升加载稳定性
...
- 弹幕加载加固：以 generation 计数避免过期异步解码覆盖新结果，预计算文本特征加速去重合并
- 修复全屏检测与退出路径：改用 AppletFrame 判定全屏状态，统一小 Activity 栈退出逻辑，避免误触返回退出全屏
- 自定义主题按 ID 直接定位文件系统路径并校验，防止路径越权
...
启动加速：视频详情请求并行化、预取播放地址，首页首帧后再预热播放器  
全屏模式：进入播放器自动全屏，PC端支持窗口全屏与应用内全屏同步  
弹幕去重：自动合并短时间窗口内重复相似的弹幕，标注重复次数  
播放线路选择：为视频与直播切换 CDN 线路，支持指定 CDN 节点  
...
- [x] 弹幕去重合并（相似度聚类，标注重复次数）
- [x] 播放线路选择（CDN 节点切换，视频与直播）
- [ ] 搜索支持搜索用户
- [ ] 长按一键三连
- [ ] 支持个人主页
- [ ] 评论跳转进度
